(function (factory) {
    ...
    var oCalenWrap = create('div', {"class" : 'calendar'}),           // 最大父级
        ...
        calendarWeekList = create('div', {"class" : 'calendar-weeklist close'}),    // 日历列表
        calendarBottom = create('div', {"class" : 'calendar-bottom'}),    // 日历底部收缩
    ...
    Calendar.prototype.init = function(){
        ...
        this.createHeader(function(){
            ...
            // 滑动切换上下周
            sildeSwitch(calendarWeekList, function(obj, dir){
                dir > 0 ? wwNow-- : wwNow++;

                $this.startWeekJSON.prev.ww = wwNow - 1;
                $this.startWeekJSON.now.ww = wwNow;
                $this.startWeekJSON.next.ww = wwNow + 1;
                $this.transitions(obj, dir,2);
            });
        ...
        if(!$this.startDate instanceof Array || !$this.startDate){
            ...
            $this.startWeekJSON.prev = { mm : mmNow, ww : wwNow - 1 };
            $this.startWeekJSON.now = { mm : mmNow, ww : wwNow };
            $this.startWeekJSON.next = { mm : mmNow, ww : wwNow + 1 };
        }
        ...
        if($this.toggle){
            // 创建收缩底部
            $this.createBottom();
            calenBtn = getElement(calendarBottom, '.calendar-btn');
            calendarBottom.onclick = function(){
                if(toolClass(calendarList, 'close','has')){
                    toolClass(calendarList, 'close','remove');
                    toolClass(calendarWeekList, 'close','add');
                    calenBtn[0].innerHTML='收缩';
                }else{
                    toolClass(calendarList, 'close','add');
                    toolClass(calendarWeekList, 'close','remove');
                    calenBtn[0].innerHTML='展开';
                }
            }
        }
        ...
        // 创建周历对象列表
        $this.appendWeekList($this.startWeekJSON, function(){
            $this.addEvent(2);
        });
    ...
    Calendar.prototype.createCalenWeekList = function(data, setTitle){
        ...
        var now = new Date(),
            ...
            nowMonth = now.getMonth()+data.mm,      //当前月
            nowDay = now.getDate()+data.ww*7,       //当前日
     ...
     * @return {[type]}      [description]
     */
    Calendar.prototype.createBottom = function(){
        var txt = '收缩';
         calendarBottom.appendChild(create('span', {"class" : 'calendar-btn'},txt));
         oCalen.appendChild(calendarBottom);
    } 
     ...
     * @return {[type]}      [description]''
     */
    Calendar.prototype.appendWeekList = function(data, cb){
        data = data || {};
        data.prev = data.prev || {ww : wwNow - 1, mm : mmNow};
        data.now = data.now || {ww : wwNow, mm : mmNow};
        data.next = data.next || {ww : wwNow + 1, mm : mmNow};

        calendarWeekList.innerHTML = '';

        calendarWeekList.appendChild(this.createCalenWeekList(data.prev));
        calendarWeekList.appendChild(this.createCalenWeekList(data.now, true));
        calendarWeekList.appendChild(this.createCalenWeekList(data.next));
     ...
     * 设置日历事件
     */
    Calendar.prototype.addEvent = function(type){
        var $this = this;
        var aCalenSet = [];
        switch(type){
            case 2:
                aCalenSet = calendarWeekList.getElementsByTagName('a');
                break;
            case 1:
            default:
                aCalenSet = calendarList.getElementsByTagName('a');
        }
