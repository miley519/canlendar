(function (factory) {
    ...
    var oCalenWrap = create('div', {"class" : 'calendar'}),           // 最大父级
        ...
        calendarToggleBtn = create('div', {"class" : 'calendar-toggle-btn'}),    // 日历切换显示隐藏
    ...
    Calendar.prototype.init = function(){
        ...
        if($this.toggle){
            // 创建收缩底部
            $this.createTogBtn();
            calenBtn = getElement(calendarToggleBtn, '.calendar-btn');
            calendarToggleBtn.onclick = function(){
                if(toolClass(calendarList, 'close','has')){
                    toolClass(calendarWeek, 'close','remove');
                    toolClass(calendarList, 'close','remove');
                    calenBtn[0].innerHTML='收起';
                }else{
                    toolClass(calendarWeek, 'close','add');
     ...
     * @return {[type]}      [description]
     */
    Calendar.prototype.createTogBtn = function(){
        var txt = '收起';
         calendarToggleBtn.appendChild(create('span', {"class" : 'calendar-btn'},txt));
         oCalen.appendChild(calendarToggleBtn);
    } 
